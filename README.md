...
- It builds `llama-embedding` and `llama-tokenize` from Hanxiao's `llama.cpp` fork (`master` branch) using CMake, because the old Makefile path is deprecated there.
